package by.yurkova.cleverbank.repository;

import by.yurkova.cleverbank.config.AppConfig;
import by.yurkova.cleverbank.exception.RepositoryException;
...
public class AccountRepository extends Repository<Account> {
    ...
    private static final String ACCRUE_INTEREST_QUERY = "UPDATE accounts SET balance = balance * ? WHERE bank_id = 1";
    ...
    /**
     * Accrues interest on account balances with the given interest rate.
     *
     * @author Yurkova Anastacia
     */
    public void accrueInterest(BigDecimal interestRate) throws RepositoryException {
        try (Connection connection = getDataSource().getConnection();
             PreparedStatement selectForUpdate = connection.prepareStatement(SELECT_ALL_FOR_UPDATE_QUERY);
             PreparedStatement updateBalance = connection.prepareStatement(ACCRUE_INTEREST_QUERY)
        ) {
            try {
     ...
     * @author Yurkova Anastacia
     */
    public Transaction refill(Account account, BigDecimal amount) {
